metadata {
	definition(name: "Tasmota-Pow", namespace: "HelloLiam", author: "Christo Steyn", ocfDeviceType: "oic.d.smartplug", vid: "generic-switch") {
        capability "Switch"
	...
	tiles(scale: 2) {
		...
		valueTile("power", "power", width: 3, height: 2) {
		 	state "default", label:'Power Usage:\n ${currentValue}W', backgroundColor: "#ffffff"
		}

        valueTile("voltage", "voltage", width: 3, height: 1) {
			state "voltage", label: 'Voltage: ${currentValue}V', backgroundColor: "#ffffff"
		}

        valueTile("current", "current", width: 3, height: 1) {
		 	state "current", label: 'Current: ${currentValue}A', backgroundColor: "#ffffff"
		}
		...
		main "switch"
		details(["switch", "refresh", "power", "voltage", "current", "ssid", "macAddress", "ipAddress", "module", "version", "hostname", "upTime"])
	}
...
def refreshCallback(physicalgraph.device.HubResponse response){
    ...
    sendEvent(name : "power", value : response?.json?.StatusSNS?.ENERGY?.Power)
    sendEvent(name : "current", value : response?.json?.StatusSNS?.ENERGY?.Current)
    sendEvent(name : "voltage", value : response?.json?.StatusSNS?.ENERGY?.Voltage)
