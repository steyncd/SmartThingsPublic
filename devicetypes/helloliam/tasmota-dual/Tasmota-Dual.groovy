metadata {
	definition(name: "Tasmota-Dual", namespace: "HelloLiam", author: "Christo Steyn", ocfDeviceType: "oic.d.smartplug", vid: "generic-switch") {
        capability "Switch"
        capability "Switch"
		capability "Refresh"
		capability "Polling"
	...
	tiles(scale: 2) {
        ...
        multiAttributeTile(name:"switch1", type:"generic", width:3, height:3) {
			tileAttribute("device.switch", key: "PRIMARY_CONTROL") {
				attributeState "on", label:'On', action:"off1", icon:"st.switches.switch.on", backgroundColor:"#00A0DC", nextState:"turningOff"
				attributeState "off", label:'Off', action:"on1", icon:"st.switches.switch.off", backgroundColor:"#ffffff", nextState:"turningOn"
				attributeState "turningOn", label:'Turning On', action:"off1", icon:"st.switches.switch.on", backgroundColor:"#00A0DC", nextState:"turningOff"
				attributeState "turningOff", label:'Turning Off', action:"on1", icon:"st.switches.switch.off", backgroundColor:"#ffffff", nextState:"turningOn"
			}
		}
        multiAttributeTile(name:"switch2", type:"generic", width:3, height:3) {
			tileAttribute("device.switch", key: "PRIMARY_CONTROL") {
				attributeState "on", label:'On', action:"off2", icon:"st.switches.switch.on", backgroundColor:"#00A0DC", nextState:"turningOff"
				attributeState "off", label:'Off', action:"on2", icon:"st.switches.switch.off", backgroundColor:"#ffffff", nextState:"turningOn"
				attributeState "turningOn", label:'Turning On', action:"off2", icon:"st.switches.switch.on", backgroundColor:"#00A0DC", nextState:"turningOff"
				attributeState "turningOff", label:'Turning Off', action:"on1", icon:"st.switches.switch.off", backgroundColor:"#ffffff", nextState:"turningOn"
			}
		}
		...
		details(["switch1", "switch2", "refresh", "ssid", "macAddress", "ipAddress", "module", "version", "hostname", "upTime"])
	}
...
def setPower(channel, power){
    ...
    sendHubCommand(createCommand("Power${channel}", power, setPowerCallback));
}
